...
static struct kobject *d64p512t_fpga;
...
static ssize_t d64p512t_fpga_reg_show(struct device *dev,
                               struct device_attribute *attr, char *buf)
{
    fpga_sysfs_attr_st *fpga_container = TO_FPGA_SYSFS_ATTR(attr);
    const d64p512t_fpga_reg_st *fpga_reg = fpga_container->fpga_reg;
    ...
    return scnprintf(buf, PAGE_SIZE,
                    ...
                    val, (fpga_reg->help) ? fpga_reg->help : "",
                    bit_offset + n_bits - 1,
                    bit_offset, offset, reg_val);

}

static ssize_t d64p512t_fpga_reg_store(struct device *dev,
                                struct device_attribute *attr, char *buf,
                                size_t count)
{
    fpga_sysfs_attr_st *fpga_container = TO_FPGA_SYSFS_ATTR(attr);
...
static int fpga_attr_create(void)
{
    ...
    int i;
    int len = sizeof(sysfpga_reg_table)/sizeof(d64p512t_fpga_reg_st);
    ...
    for(i = 0; i < len; i++, fpga_container++)
    {
        ...
        fpga_container->dev_attr.show = (void *)d64p512t_fpga_reg_show;
        fpga_container->dev_attr.store = (void *)d64p512t_fpga_reg_store;
...
int fpga_attr_init(struct pci_dev *dev, struct fpga_dev *fpga)
{
    ...
    d64p512t_fpga = kobject_create_and_add("d64p512t_fpga", kernel_kobj);
    if (!d64p512t_fpga)
        return -ENOMEM;
    fpga_attr_create();

    mutex_init(&fpga_attr_lock);
    error = sysfs_create_group(d64p512t_fpga,&attr_grp);
    if (error)
    {
        kobject_put(d64p512t_fpga);
        pr_info("failed to create the d64p512t_fpga_reg file "
                "in /sys/kernel/d64p512t_fpga\n");
    }
    return 0;
}
