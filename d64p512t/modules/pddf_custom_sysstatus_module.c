...
PDDF_DATA_ATTR(cpu_cpld_version, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(cpu_cpld_board_info, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(cpld_a_version, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(cpld_a_board_info, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(cpld_b_version, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(cpld_b_board_info, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(power_status1, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(power_status2, S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(watchdog_timeout, S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(watchdog_enable, S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(watchdog_clear, S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(reset_cause, S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);

static struct attribute *sysstatus_data_attributes[] = {
    &attr_cpu_cpld_version.dev_attr.attr,
    &attr_cpu_cpld_board_info.dev_attr.attr,
    &attr_cpld_a_version.dev_attr.attr,
    &attr_cpld_a_board_info.dev_attr.attr,
    &attr_cpld_b_version.dev_attr.attr,
    &attr_cpld_b_board_info.dev_attr.attr,
    &attr_power_status1.dev_attr.attr,
    &attr_power_status2.dev_attr.attr,
    &attr_watchdog_timeout.dev_attr.attr,
    &attr_watchdog_enable.dev_attr.attr,
    &attr_watchdog_clear.dev_attr.attr,
    &attr_reset_cause.dev_attr.attr,
    NULL
};
...
ssize_t show_sysstatus_data(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    else
    {
        ...
        mask  = sysstatus_addr_attrs->mask;
        while ((mask & 0x01) != 1)
        {
            mask = mask >> 1;
            idx++;
    ...
    return sprintf(buf, "%d\n", ((status & sysstatus_addr_attrs->mask) >> idx));
...
ssize_t store_sysstatus_data(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    else
    {
        ...
        // Read value and set the new value with mask
        status = board_i2c_cpld_read( sysstatus_addr_attrs->devaddr, sysstatus_addr_attrs->offset);
        mask  = sysstatus_addr_attrs->mask;
        while ((mask & 0x01) != 1)
        {
        ...
        reg_val = (reg_val << idx) & sysstatus_addr_attrs->mask;
        reg_val = (status & (~sysstatus_addr_attrs->mask & 0xff)) | reg_val;
...
MODULE_AUTHOR("Parthiv Shah <parthiv@marvell.com>");
