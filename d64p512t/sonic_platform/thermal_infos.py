    ...
    def collect(self, chassis):
        ...
        for index in range(num_of_thermals):
            ...
            if high_critical_threshold and temp > high_critical_threshold:
                ...
                self._critical_thermals.add(thermal)
            else:
                self._over_high_critical_threshold = False
                self._critical_thermals.discard(thermal)

    def get_fault_thermals(self):
        """
        Retrieves the temperature sensors whose temperature is over critical threshold value
        :return: set of thresholds whose temperature value is over critical threshold
        """
        return self._critical_thermals
...
class PsuInfo(ThermalPolicyInfoBase):
    ...
    def collect(self, chassis):
        ...
        for psu in chassis.get_all_psus():
            ...
            if presence and psu not in self._presence_psus:
                self._presence_psus.add(psu)
                self._status_changed = True
                if psu in self._absence_psus:
                    self._absence_psus.remove(psu)
            elif not presence and psu not in self._absence_psus:
                self._absence_psus.add(psu)
                self._status_changed = True
                if psu in self._presence_psus:
                    self._presence_psus.remove(psu)

            if not status and psu not in self._fault_psus:
                self._fault_psus.add(psu)
                self._status_changed = True

            elif status and psu in self._fault_psus:
                self._fault_psus.remove(psu)
                self._status_changed = True

    def get_absence_psus(self):
        """
        Retrieves absence psus
    ...
    def get_presence_psus(self):
        ...
        :return: A set of presence psus
        """
        return self._presence_psus

    def get_fault_psus(self):
        """
        Retrieves fault psus
        :return: A set of fault psus
        """
        return self._fault_psus
